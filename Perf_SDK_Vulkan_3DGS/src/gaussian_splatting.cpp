...
void GaussianSplatting::onRender(VkCommandBuffer cmd)
{
  ...
  static bool frameStarted = false;
    if (!frameStarted) {
        m_nvperf.OnFrameStart(m_queue);
        frameStarted = true;
    }
  // simulating an on frame start method for nvperf.
  ...
  if(splatCount)
  {
    ...
    {
      m_nvperf.rangeCommands.PushRange(cmd, "A_GPU_Radix_Sort");
...
void GaussianSplatting::updateAndUploadFrameInfoUBO(VkCommandBuffer cmd, const uint32_t splatCount)
{
  ...
  // Add automatic rotation control
  static float rotationAngle = 0.0f;
  static float rotationSpeed = 0.5f; // radians per second
  static bool autoRotate = true; // Set to false if you want manual control
  ...
  if(autoRotate)
  {
    ...
    // Set camera position in a circular orbit
    float radius = 5.0f; // Distance from origin
    float height = 2.0f; // Height above origin
    m_eye = glm::vec3(radius * sin(rotationAngle), height, radius * cos(rotationAngle));
    m_center = glm::vec3(0.0f); // Look at origin
    m_up = glm::vec3(0.0f, 1.0f, 0.0f); // Up vector
    
    // Update camera manipulator
    CameraManip.setLookat(m_eye, m_center, m_up);
  }
  else
  {
...
void GaussianSplatting::drawSplatPrimitives(VkCommandBuffer cmd, const uint32_t splatCount)
{
  ...
  m_nvperf.rangeCommands.PushRange(cmd, "DrawSplatsMeshOrVert");
