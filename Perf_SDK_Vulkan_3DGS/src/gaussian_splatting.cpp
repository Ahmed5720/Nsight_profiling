...
void GaussianSplatting::onAttach(nvapp::Application* app)
{
  ...
  m_instance = m_app->getInstance();
  m_physicalDevice = m_app->getPhysicalDevice();
  m_queue = m_app->getQueue(0).queue;
  ...
  PFN_vkGetInstanceProcAddr pfnVkGetInstanceProcAddr = (PFN_vkGetInstanceProcAddr)vkGetInstanceProcAddr;
  PFN_vkGetDeviceProcAddr pfnVkGetDeviceProcAddr = (PFN_vkGetDeviceProcAddr)vkGetDeviceProcAddr;

  m_nvperf.InitializeReportGenerator(m_instance, m_physicalDevice, m_device, pfnVkGetInstanceProcAddr, pfnVkGetDeviceProcAddr);
  //m_nvperf.InitializeReportGenerator(m_instance, m_physicalDevice, m_device);
  m_nvperf.SetFrameLevelRangeName("GaussianSplatting_Frame");
  m_nvperf.SetNumNestingLevels(2); // Adjust based on your nesting depth
  m_nvperf.SetMaxNumRanges(3);

  m_nvperf.outputOptions.directoryName = "HtmlReports\\gaussian_splatttt";
...
void GaussianSplatting::onRender(VkCommandBuffer cmd)
{
  ...
  m_nvperf.OnFrameStart(m_queue, m_app->getQueue(0).familyIndex,pfnVkGetInstanceProcAddr, pfnVkGetDeviceProcAddr);
  ...
  //#ifdef NV_PERF_ENABLE_INSTRUMENTATION
  // m_nvperf.rangeCommands.PushRange(cmd, "GaussianSplatting_Render");
  //#endif
  ...
  updateRenderingMemoryStatistics(cmd, splatCount);
      ...
      m_nvperf.OnFrameEnd();
      //vkQueueWaitIdle(m_queue);  // Required workaround
        ...
        {
          std::cout << "Nsight Perf: Currently profiling the frame. HTML Report will be written to: <CWD>\\somewhere\n";
        }
        
        else if (m_nvperf.GetInitStatus() == nv::perf::profiler::ReportGeneratorInitStatus::Succeeded)
        {
          std::cout << "Nsight Perf: Hit <space-bar> to begin profiling.\n";
        }
        else
        {
          std::cout << "Nsight Perf: Initialization failed. Please check the logs\n.";
        }
    

  //m_nvperf.rangeCommands.PopRange(cmd);
  #endif
...
void GaussianSplatting::updateAndUploadFrameInfoUBO(VkCommandBuffer cmd, const uint32_t splatCount)
{
  ...
  #ifdef NV_PERF_ENABLE_INSTRUMENTATION
  m_nvperf.rangeCommands.PushRange(cmd, "UpdateFrameInfo");
  #endif
  NVVK_DBG_SCOPE(cmd);
...
void GaussianSplatting::processSortingOnGPU(VkCommandBuffer cmd, const uint32_t splatCount)
{
  ...
  #ifdef NV_PERF_ENABLE_INSTRUMENTATION
  m_nvperf.rangeCommands.PushRange(cmd, "GPU_Sorting");
  #endif
  NVVK_DBG_SCOPE(cmd);
...
{
#ifdef NV_PERF_ENABLE_INSTRUMENTATION
  m_nvperf.rangeCommands.PushRange(cmd, "DrawSplats");
  #endif
  NVVK_DBG_SCOPE(cmd);
