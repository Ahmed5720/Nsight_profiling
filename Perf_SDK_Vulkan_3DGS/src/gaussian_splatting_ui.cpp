...
void GaussianSplatting::onUIRender()
{
  ...
  if (ImGui::CollapsingHeader("Profiling")) {
        if (ImGui::Button("Start Perfer Profiling")) {
            std::string outDir = "./profiling_output";
            m_profilingEnabled = m_nvperf.StartCollectionOnNextFrame(outDir, AppendDateTime::yes);
        }
        
        ImGui::SameLine();
        if (ImGui::Button("Stop Profiling")) {
            m_profilingEnabled = false;
            // Profiling will automatically stop after the configured number of frames
        }
        
        ImGui::Text("Profiling Status: %s", m_profilingEnabled ? "Active" : "Inactive");
    }
