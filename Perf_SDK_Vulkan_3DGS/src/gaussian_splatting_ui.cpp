...
void GaussianSplatting::onUIRender()
{
  ...
  // Keyboard input handling for profiling (only when not typing in input fields)
  if (!ImGui::GetIO().WantTextInput && ImGui::IsKeyPressed(ImGuiKey_Space)) {
  #ifdef NV_PERF_ENABLE_INSTRUMENTATION
      std::cout << "perfer enabled\n";
      nv::perf::InitializeNvPerf();
      if (!m_profilingEnabled) {
        // Start profiling
        std::string outDir = "HtmlReports";
        m_profilingEnabled = m_nvperf.StartCollectionOnNextFrame(); //outDir,  nv::perf::AppendDateTime::yes);
        if (m_profilingEnabled) {
          std::cout << "Profiling started via spacebar, output: " << outDir << std::endl;
        } else {
          std::cout << "Failed to start profiling via spacebar" << std::endl;
        }
      } else {
        // Stop profiling
        m_profilingEnabled = false;
        std::cout << "Profiling stopped via spacebar" << std::endl;
      }
  #else
      std::cout << "Profiling not available - compile with NV_PERF_ENABLE_INSTRUMENTATION" << std::endl;
  #endif
  }
