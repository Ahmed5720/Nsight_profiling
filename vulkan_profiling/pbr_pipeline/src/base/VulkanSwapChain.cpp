...
void VulkanSwapChain::initSurface(GLFWwindow* window) 
{
	...
	// this->presentQueueNodeIndex = presentQueueNodeIndex;
	...
	for (auto format : surfaceFormats) {
		std::cout << "Available swapchain format: " << format.format << std::endl;
	}
	...
	std::cout << "Selected swapchain color format: " << colorFormat << std::endl;
...
VkResult VulkanSwapChain::queuePresent(VkQueue queue, uint32_t imageIndex, VkSemaphore waitSemaphore)
{
	...
	{
		// std::cout << "Waiting for semaphore before presenting image" << std::endl;
