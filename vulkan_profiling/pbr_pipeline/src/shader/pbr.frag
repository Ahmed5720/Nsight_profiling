...
// layout (location = 12) in vec2 inUVdebug;
...
{	
    ...
    for(int i = 0; i < 6; ++i) 
    {
        ...
        if (dotNL > 0.0) {
            ...
            Lo += (kD * albedoColor / PI + brdf) * radiance * NdotL;  // note that we already multiplied the BRDF by the Fresnel (kS) so we won't multiply by kS again
        }
    }
