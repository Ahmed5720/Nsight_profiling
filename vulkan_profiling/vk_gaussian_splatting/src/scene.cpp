...
void gltfr::Scene::init(nvh::gltf::Scene* gltfScene, nvvkhl::SceneVk* gltfSceneVk, nvvk::ResourceAllocator* resAlloc)
{
  m_alloc = resAlloc;
  ...
  m_gltfSceneVk = gltfSceneVk;  // Use the existing Vulkan
  createDescriptorPool(m_alloc->getDevice());
  createDescriptorSet(m_alloc->getDevice());
}
...
// - Reset the sky
void gltfr::Scene::deinit()
{
  // res.m_allocator->destroy(m_sceneFrameInfoBuffer);

  destroyDescriptorSet(m_alloc->getDevice());
...
void gltfr::Scene::selectRenderNode(int renderNodeIndex)
{
  ...
  if(m_gltfScene && renderNodeIndex > -1)
  {
