...
public:  // Methods specializing IAppElement
  GaussianSplatting(std::shared_ptr<nvvkhl::ElementProfiler> profiler, std::shared_ptr<argparse::ArgumentParser> parser);
  ...
  // set matrices for the camera
  void setCameraMatrices(const float* view, const float* proj, const float* model);
...
private:  // Attributes
  ...
  std::string m_sceneToLoadGltfFilename;  
  // name of the loaded scene if successfull
  std::string m_loadedSceneFilename;
  std::string m_loadedSceneGltfFilename;

  //output file name
  std::string m_outputFilename;
  bool       m_outputScreenshot = false;
  int fc = 0;  // frame count for screenshot
  ...
  std::unique_ptr<nvvk::DescriptorSetContainer> m_dset_pbr;                        // PBR Descriptor set
  std::unique_ptr<nvh::gltf::Scene>        m_gltfScene{};                          // The glTF scene
  std::unique_ptr<nvvkhl::SceneVk>    m_gltfSceneVk{};                         // The glTF scene in Vulkan format
  std::unique_ptr<nvvk::ResourceAllocator> m_resAlloc{};                               // Resource allocator
  ...
  struct Shaders
  {
    ...
    //PBR shaders
    nvvk::ShaderModuleID pbrVertexShader;
    nvvk::ShaderModuleID pbrFragmentShader;
  } m_shaders;
