...
//   benchmark->addPostBenchmarkAdvanceCallback([&]() { benchmarkAdvance(); });
  if (parser->is_used("input1"))  {
    m_sceneToLoadFilename = parser->get<std::string>("input1");
  }
  if (parser->is_used("output")) {
    m_outputFilename = parser->get<std::string>("output");
    m_outputScreenshot = true;
  }
  if (parser->is_used("view")) {
    std::vector<float> view = parser->get<std::vector<float>>("view");
    if (view.size() == 16) {
      view_cust = glm::make_mat4(view.data());
    }
  }
  if (parser->is_used("proj")) {
    std::vector<float> proj = parser->get<std::vector<float>>("proj");
    if (proj.size() == 16) {
      proj_cust = glm::make_mat4(proj.data());
    }
  }
  ...
  glm::mat4 temp = glm::inverse(view_cust);
	...
	eye_cust = campos;
  // m_defines.maxShDegree = 3;
  // m_defines.shFormat    = FORMAT_FLOAT32;
  m_selectedPipeline    = PIPELINE_VERT;
...
void GaussianSplatting::onAttach(nvvkhl::Application* app)
{
  ...
  m_dset_pbr = std::make_unique<nvvk::DescriptorSetContainer>(m_device);
...
void GaussianSplatting::setCameraMatrices(const float* view, const float* proj, const float* model)
{
...
void GaussianSplatting::onRender(VkCommandBuffer cmd)
{
  ...
  if(m_outputScreenshot && splatCount > 0) {
    fc++;
    if (fc == 10) 
      m_app->screenShot(m_outputFilename, 100);
...
void GaussianSplatting::onLastHeadlessFrame()
{
  // take a screenshot
  // if(!m_outputFilename.empty())
  // {
  //   VkImage srcImage = ;
  //   VkExtent2D imageSize = m_gBuffers->getSize();
  //   m_app->saveImageToFile(srcImage, imageSize, m_outputFilename, 100);
  //   // m_app->saveScreenShot(m_outputFilename, 100);
  //   // std::cout << "Screenshot saved to " << m_outputFilename << std::endl;
  // }
}
...
void GaussianSplatting::updateAndUploadFrameInfoUBO(VkCommandBuffer cmd, const uint32_t splatCount)
{
  ...
  m_frameInfo.cameraPosition         = eye_cust;
...
bool GaussianSplatting::initShaders(void)
{
  ...
  // generate the 3dgs shader modules
  ...
  // generate the pbr shader modules
  m_shaders.pbrVertexShader = m_shaderManager.createShaderModule(VK_SHADER_STAGE_VERTEX_BIT, "pbr.vert.glsl", "");
  m_shaders.pbrFragmentShader = m_shaderManager.createShaderModule(VK_SHADER_STAGE_FRAGMENT_BIT, "pbr.frag.glsl", "");
...
{
  // reset descriptor bindings for 3dgs
  ...
  // bindinbgs for PBR
  m_dset_pbr->setBindings(empty);

  m_dset_pbr->addBinding(BINDING_FRAME_INFO_UBO, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1, VK_SHADER_STAGE_ALL);
