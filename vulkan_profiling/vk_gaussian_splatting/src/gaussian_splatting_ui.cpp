...
// #endif
  ...
  //Do we need to load a new glTF scene ?
  if(!m_sceneToLoadGltfFilename.empty())
  {
    m_loadedSceneGltfFilename = m_sceneToLoadGltfFilename;
    if(m_gltfScene->load(m_sceneToLoadGltfFilename))
    {
      // sceneloaded  = true;
      // m_sceneGraph = std::make_unique<GltfModelUI>(m_gltfScene->getModel(), m_gltfScene->getSceneBounds());
      // createVulkanScene(resources);
      m_resAlloc = std::make_unique<nvvk::ResourceAllocator>(m_app->getDevice(), m_app->getPhysicalDevice(), m_alloc->getMemoryAllocator());

      m_gltfSceneVk = std::make_unique<nvvkhl::SceneVk>(m_resAlloc->getDevice(), m_resAlloc->getPhysicalDevice(), m_resAlloc.get());
      ...
      {  // Creating the scene in Vulkan buffers
        ...
        m_resAlloc->finalizeAndReleaseStaging();  // Make sure there are no pending staging buffers and clear them up
      }
    }
    else
    {
      std::cout << "Error: cannot load glTF scene " << m_sceneToLoadGltfFilename << std::endl;
    }
    m_sceneToLoadGltfFilename.clear();
  }
